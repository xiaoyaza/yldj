...
public class TradingServiceImpl extends ServiceImpl<TradingMapper, Trading> implements TradingService {
     ...
     */
    @Override
    public Trading queryDuringTrading(String productAppId,String productOrderNo, String tradingChannel){
        LambdaQueryWrapper<Trading> queryWrapper = new LambdaQueryWrapper<>();
     ...
     */
    @Override
    public List<Trading> queryByProductOrder(String productAppId,String productOrderNo){
        LambdaQueryWrapper<Trading> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(Trading::getProductOrderNo, productOrderNo)
                .eq(Trading::getProductAppId, productAppId);
        return list(queryWrapper);
    ...
    /**
     * 根据订单id查询已付款的交易单，如果没有已付款的交易单则远程请求第三方支付系统查询
     *
     * @param productOrderNo 订单id
     * @return 交易单
     */
    @Override
    public Trading findFinishedTrading(String productAppId,String productOrderNo){
        LambdaQueryWrapper<Trading> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(Trading::getProductOrderNo, productOrderNo)
                .eq(Trading::getProductAppId,productAppId);
        List<Trading> list = list(queryWrapper);
        //使用stream流从list中提取已付款的交易单
        Trading first = list.stream().filter(trading -> trading.getTradingState().equals(TradingStateEnum.YJS)).findFirst().orElse(null);
        if (first != null) {
            return first;
        }
        //远程调用第三方支付系统查询支付结果
        for (Trading trading : list) {
            try {
                TradingDTO tradingResult = basicPayService.queryTradingResult(trading.getTradingOrderNo());
                if(ObjectUtil.isNotNull(tradingResult)&&tradingResult.getTradingState().equals(TradingStateEnum.YJS)){
                    BeanUtil.copyProperties(tradingResult, trading);
                    return trading;
                }
